	 ...
	 public void creating_workflow_based_on_the_type_selected_in_the_select_category_of_the_project(String sheetname, Integer startRownumber) throws Exception {
	         ...
	         while (rownumber < categoryData.size()) {
	          ...
	          // Fetch the value for "Name for approval"
	             ...
	             // Check if this is an RFI approval (where "Name for approval" is disabled with default value "Answered")
	             if (nameForApproval != null && nameForApproval.equals("Answered")) {
	                 ...
	                 if (actualValueInField.equals("Answered")) {
	                     System.out.println("Confirmed that 'Name for Approval' contains 'Answered'. Skipping entry as it's disabled for RFI approval.");
	                 } else {
	                     throw new Exception("Expected 'Name for Approval' to be 'Answered' but found: " + actualValueInField);
	                 }
	          ...
	          // Retrieve the Editable value from Excel and parse it as an integer
	             ...
	             // Check if the editableStatus is 1, then click the checkbox; otherwise, do nothing
	             if (editableStatus == 1) {
	 ...
	 public void fillApprovalSteps(String sheetname, int rownumber) throws Exception {
		    ...
		    for (int step = 1; step <= numberOfSteps; step++) {
		        ...
		        if (stepName != null && !stepName.isEmpty()) {
		            ...
		            if (step >= 3 && step <= 7) {
		                String stepReviseColumn = "Step" + step + "Revise"; // Assuming you have columns like "Step3Revise", "Step4Revise", etc.
		                ...
		                if (stepReviseValue != null && !stepReviseValue.isEmpty()) {
		                	if (stepReviseValue != null && stepReviseValue.matches("\\d+\\.0")) {
			            		...
			            		System.out.println(step + "--->step NO");
...
//		}
		...
		public void selectDropdownFromApprovalLimit(String ApprovalLimit, int step) throws Exception {
		    ...
		    List<WebElement> dropdownOptions = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//ul[@id='name" + step + "_options']/li")));
		    boolean optionFound = false;
	 ...
	 public void EnterOnTimeAllowed(String Time, int timeallow) {
		    ...
		    ((JavascriptExecutor) driver).executeScript("arguments[0].value=arguments[1];", Field, Time); // Set the new value
	 ...
	 public void selectDropdownFromCalendar(String calendar, int calen) {
		    ...
		    String optionXPath = dropdownXPath + "//option[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '" 
		    ...
		    try {
		        ...
		        System.out.println("The dropdown option '" + calendar + "' is not found or not clickable within the timeout.");
	 ...
	 public void AutoApprovalDays(String Days, int approvalstep) {
		    ...
		    try {
		        ...
		        System.out.println("AutoApprovalDays field not available or interactable for step " + approvalstep + ". Skipping...");
		    }
	...
	//Select Users
	 ...
	 public void selectDropdownUSERS(String userName, int step) throws Exception {
		    ...
		    List<WebElement> dropdownOptions = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//ul[@id='multiUser" + step + "_options']/li")));
		    boolean optionFound = false;
		    for (WebElement option : dropdownOptions) {
		        String txt = option.getText();
		        System.out.println(option);
	...
	public void selectDropdownFromMultiUserGroup(String userName, int step) throws Exception {
	    ...
	    List<WebElement> dropdownOptions = wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//ul[@id='multiUserGroup" + step + "_options']/li")));
	    boolean optionFound = false;
	    for (WebElement option : dropdownOptions) {
	        String txt = option.getText();
	        System.out.println(option);
